...
sleep 5
echo "$(tput setaf 1)Creating admin user"
tput setaf 7
mongo <<'EOF'
use admin
...
exit
EOF
echo "$(tput setaf 1)Creating mongo key"
tput setaf 7
openssl rand -base64 756 > /opt/mongo-keyfile
chmod 400 /opt/mongo-keyfile
chown mongodb:mongodb /opt/mongo-keyfile
echo "$(</opt/mongo-keyfile)"
echo "$(tput setaf 1)copy this key and paste to other servers"
tput setaf 7
